...
word_ttf = ImageFont.truetype("./source/font/T1.ttf", 109)
word_use = set(TTFont("./source/font/T1.ttf")["cmap"].tables[0].ttFont.getBestCmap())
emoji_ttf = ImageFont.truetype("./source/font/NotoColorEmoji.ttf", 109)
emoji_use = set(
    TTFont("./source/font/NotoColorEmoji.ttf")["cmap"].tables[0].ttFont.getBestCmap()
)
...
class EmojiWriter:
    ...
    def __init__(self, font=None):
        ...
        self.word_use = (
            word_use
            if font is None
            else set(TTFont(font)["cmap"].tables[0].ttFont.getBestCmap())
        )
    ...
    def get_size(self, text="", size=None, stroke_width=0, width=None, use=None):
        ...
        if use is None:
            use = (
                self._steady_width_split(text, size, width)
                if width
                else self.analyze(text)
            )
        data = [self._get_size(size, line) for line in use]
        if len(data) == 1:
            return data[0]
        else:
            le = int(max(a[0] for a in data))
    ...
    def _get_size(self, size, use, width=None):
        ...
        return tuple(
            map(lambda x: int(x * (size or self.size) / 109), (length, height))
        )
    ...
    def text2pic(self, text, color, size=None, stroke_width=0, width=None):
        ...
        其中可能有些width会变大/变小"""
        use = (
            self._steady_width_split(text, size, width) if width else self.analyze(text)
        )
        all_size = [
            [word["type"].getsize(word["words"]) for word in line] for line in use
        ]
        font_height = int(
            max(char[1] for line in all_size for char in line) + (stroke_width * 109 / size)
        )
        length, height = self.get_size(
            "", 109, stroke_width * 109 / size, width=width, use=use
        )
        if width:
            length = int(width * 109 / size)
        ...
        for line, line_size in zip(use, all_size):
            for w, s in zip(line, line_size):
                draw.text(
                    (start_x, start_y - w["gap"]),
                    w["words"],
                    font=w["type"],
                    fill=color,
                    embedded_color=w["type"] is emoji_ttf,
                )
                start_x += s[0]
            start_y += font_height
            start_x = 0
        ...
        return background.resize(
            map(lambda x: int(x * size / 109), background.size), IMG.ANTIALIAS
        )
